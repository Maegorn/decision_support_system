...
class Node:
    ...
    def setCost(self):  #compute the number of tiles out of place
        #cost as h1
        i = 1 #initialized at 1 cause the element in the position [0] of the solution is 1
        cost = 0
        while i < len(str(self.puzzle)):
            if i != int(str(self.puzzle)[i-1]): #compare the index number with the current puzzle state
                cost = cost + 1
            i = i+1
        self.cost = cost
